...
def main():
  ...
  result = min([convert(humidity_to_location_map, convert(temperature_to_humidity_map, convert(light_to_temperature_map, convert(water_to_light, convert(fertilizer_to_water, convert(soil_to_fertilizer_map, convert(seed_to_soil_map, seed))))))) for seed in seeds])

  assert result == 322500873
