...
def main():
    seeds = [
        int(x) for x in "".join([line.strip() for line in open("input.txt")]).split()
    ]
    ...
    for idx in range(0, len(seeds), 2):
        seed_maps.append((seeds[idx], seeds[idx + 1]))

    for x, y in seed_maps:
        print(x, y)

    mmin = float("inf")
    for start_range, range_size in seed_maps:
        for seed in range(start_range, start_range + range_size + 1):
